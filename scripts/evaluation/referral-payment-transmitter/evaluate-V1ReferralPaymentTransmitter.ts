...
async function main() {
  ...
  const deployedReferralContract: V1ReferralPaymentTransmitter =
    (await referralContract.deploy(
      receiver.address,
      PAYMENT_AMOUNT,
      REFERRAL_REWARD
    )) as V1ReferralPaymentTransmitter;
  ...
  for (let i = 0; i < loopIterations; i++) {
    ...
    const refereeUserBalanceAfter = await refereeUser.getBalance();
    console.log("Referee Balance After:", refereeUserBalanceAfter);

    const referrerBalanceAfter = await referrerUser.getBalance();
