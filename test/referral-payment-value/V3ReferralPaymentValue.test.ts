...
const DEFAULT_REFEREE_REWARD_PERCENTAGE: PercentageType = 30;
...
describe(`Testing ${CONTRACT_NAME} Referral Contract`, async () => {
  ...
  describe(`Testing Updating Contract Values`, async () => {
    ...
    it(`${CONTRACT_NAME} should update referee reward allocation percentage`, async () => {
      ...
      for (const updatedValue of validUpdateValues) {
        ...
        const contractValue: BigNumber =
          await proxyContract.refereeRewardPercentage();
        expect(contractValue).to.equal(updatedValue);
      }
    });
  ...
  describe(`Testing Function Modifiers`, async () => {
    it(`${CONTRACT_NAME} should throw if non-admin tries to update contract variables`, async () => {
      ...
      const validPercentage: PercentageType = 20;
      ...
      const refereeRewardUpdatePromise: Promise<ContractTransaction> =
        proxyContract
          .connect(nonAdminSigner)
  ...
  describe(`Testing Referral Process Functionality`, async () => {
    ...
    const ptRefereeRewardPercentage: PercentageType = 15;
    ...
    it(`${CONTRACT_NAME} should send the reward to the referee and referrer if referral process is completed`, async () => {
      ...
      const paymentValue: BigNumber = ptPaymentsValueThreshold;
      await executeReferralPayment({
        executions: numberOfPaymentTransactions.toNumber() - 1,
        referee,
        referrer,
        proxyContract,
        paymentValue: paymentValue,
      ...
      const expectedTotalPaidOutReward: BigNumber = paymentValue
        .mul(ptRewardPercentage)
        .div(100)
        .mul(numberOfPaymentTransactions);

      const refereeReward: BigNumber = expectedTotalPaidOutReward
        .mul(ptRefereeRewardPercentage)
        .div(100);

      const referrerReward: BigNumber =
        expectedTotalPaidOutReward.sub(refereeReward);
      ...
      const referrerResult: BigNumber =
        initialReferrerBalance.add(referrerReward);
